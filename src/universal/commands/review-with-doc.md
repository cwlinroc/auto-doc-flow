...
3. **Code review.** Look for: correctness issues, unclear assumptions, edge cases that look
   unhandled, naming inconsistencies, anything that would surprise a future reader. **Also,
   verify that the code changes align with and do not violate terminology defined in
...
4. **On finding an issue:** Ask before fixing. *"Found X — want me to fix it?"* After an
   approved fix: ask again before touching docs. *"This fix changes the contract for Y.
